from exact.five.hamil import eig_even_odd
import numpy as np
...
def gen_points():
    ...
    min_Eint = -100
    max_Eint = 100
    Ejs = np.linspace(min_Ej, max_Ej, 20)
    num_random = 100
    # we have 16 parameters in 5T
    points = np.zeros(shape=(len(Ejs) + num_random, 16))
    i = 0
    for E in Ejs:
        points[i, :] = [Ec, Ec, Ec, Ec, Ec, E, E, E, E, E, max_Eint, max_Eint, max_Eint, max_Eint, max_Eint, max_Eint]
        i += 1
    for _ in range(num_random):
        Ecs = [random.randint(min_Ec, max_Ec) for _ in range(5)]
        Ejs = [random.randint(min_Ej, max_Ej) for _ in range(5)]
        Eints = [round(random.random() * (max_Eint - min_Eint) + min_Eint, 2) for _ in range(6)]
...
def collect():
    ...
    print(points)
    num_levels = 100  # number of levels above ground to include 1 2 3 excitation subspace
    ...
    for p_i, p in enumerate(points):
        ...
        for j, M in enumerate(MM):
            ...
            levels[j, :] = vals[1 : num_levels + 1] - vals[0]  # take levels relative to ground
        final = levels[-1, :]
        relerr = (levels - final) / final  # all relative to their final
        relative_errs[p_i, :, :] = relerr

    maxed = np.max(np.abs(relative_errs), axis=(0, 2))  # max over parameter point and energy level
    np.save("converce_5T_maxed", maxed)


def plot():
    maxed = np.load("converce_5T_maxed.npy") + 1e-30

    plt.semilogy(MM[:-1], maxed[:-1])
    plt.title(f"Relative error, C={C}")
